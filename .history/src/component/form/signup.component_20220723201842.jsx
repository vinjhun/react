...
import { FirebaseAuthUtil, signUpWithEmail2 } from '../../util/firebase/firebase-auth';
...
const SignUpForm = () => {
  ...
  const submitHandler = async (event) => {
    ...
    try {
      ...
      const rsp = await FirebaseAuthUtil.createUserWithEmailAndPassword(email, password);

      console.log('create user');
      if (rsp) {
        rsp = await createUserFromAuth(rsp, display_name);
      }
    } catch (err) {
      console.log(err);
